...
- **↑ Price alerts mode** — shows all items where a past purchase is more expensive than the current price; sorted by savings opportunity. Five sub-tabs: **All**, **↺ Price match** (purchases ≤30 days old — Costco's price-adjustment window), **✓ Claimed**, **🔕 Acknowledged**, and **🚫 Excluded**. A legend at the top of All/Price match spells out what each icon does. Each item's purchase history is shown expanded by default; every past purchase (not just the priciest one) gets its own **↩ Return** (always available) / **↺ Match** (only within the 30-day window) buttons — claiming one removes just that specific line item on the next load, a different still-unclaimed purchase of the same item can still surface later. Never inferred from scanned receipts (a return can happen for unrelated reasons, and a price match produces no receipt at all) — always a manual, human tap. The headline row's **↩ Check refunds →** jumps to the Refunds tab pre-searched by that item's code, to check whether it's already gone through as an actual return. Two ways to quiet an item down, for different reasons: **🔕 Acknowledge** dismisses it *for now* — it reappears on its own the moment a newer purchase of that item exists, no manual re-check needed — for an otherwise-normal item whose current gap just isn't worth acting on; **🚫 Exclude** hides it *permanently*, for items that structurally don't fit this feature at all (gold bullion, gas — market-priced, never a "you got overcharged" signal). Claims land in the **Claimed** tab (running count + total value, **✕ Discard** to undo); acknowledgments land in **Acknowledged** (list + **✕ Remove** to undo early); exclusions land in **Excluded** (list + **✕ Remove** to undo) — no manual add form for either, the row buttons are the only way to add one
...
price_alert_claims:
  id, receipt_item_id, receipt_id,
  item_code, item_name,  -- item_code nullable (not every business prints one); item_name always set
...
price_alert_exclusions:
  id, item_code, item_name,  -- both set by the 🚫 row button (name is display-only)
  created_at
  -- Only added via the 🚫 button on a Price Alerts row — no manual add
  -- form. Filtered directly inside get_return_candidates() by item_code.

price_alert_acknowledgments:
  id, item_code, acknowledged_at
  -- unique (item_code). Self-expiring sibling to exclusions: suppressed in
  -- get_return_candidates() only while the item's most recent purchase is
  -- no newer than acknowledged_at — a newer purchase un-suppresses it
